...
public class PagingUtilAjax {
	 ...
	 * */
	public PagingUtilAjax(int currentPage, int totalCount, int rowCount,
	...
	public PagingUtilAjax(String keyfield, String keyword, int currentPage, int totalCount, int rowCount,
			int pageCount,String pageUrl) {
		this(keyfield,keyword,currentPage,totalCount,rowCount,pageCount,pageUrl,null);
	}
	public PagingUtilAjax(String keyfield, String keyword, int currentPage, int totalCount, int rowCount,
			int pageCount){
		this(keyfield, keyword, currentPage, totalCount, rowCount, pageCount, null, null);
	}
	public PagingUtilAjax(String keyfield, String keyword, int currentPage, int totalCount, int rowCount,
			int pageCount,String pageUrl,String addKey) {
		...
		if (currentPage > pageCount) {
			if(keyword==null){//검색 미사용시
				pagingHtml.append("<a target='"+(startPage-1)+"'>");
			}else{
				pagingHtml.append("<a target='"+(startPage-1)+"'>");
			}
		...
		if (totalPage - startPage >= pageCount) {
			if(keyword==null){//검색 미사용시
				pagingHtml.append("<a target='"+ (endPage + 1) +"'>");
			}else{
				pagingHtml.append("<a target='"+ (endPage + 1) +"'>");
			}
